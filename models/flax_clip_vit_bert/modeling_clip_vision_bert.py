...
class FlaxCLIPVisionBertEmbeddings(nn.Module):
    ...
    def setup(self):
        ...
        self.clip_vision_module = FlaxCLIPVisionModule(clip_vision_config, dtype=self.dtype)
...
class FlaxCLIPVisionBertModel(FlaxPreTrainedModel):
    ...
    def __init__(
        self, config: CLIPVisionBertConfig, input_shape: Tuple = None, seed: int = 0, dtype: jnp.dtype = jnp.float32, **kwargs
    ):

        if input_shape is None:
            input_shape = ((1, 1), (1, config.clip_vision_config.image_size, config.clip_vision_config.image_size, 3), (1, 197))
    ...
    def from_bert_clip_vision_pretrained(
        ...
        **kwargs,
        ...
        config = CLIPVisionBertConfig.from_bert_clip_vision_configs(bert_model.config, clip_vision_model.config, **kwargs)
        ...
        for key in model.params.keys():
            if key != "embeddings":
                ...
                model.params["embeddings"]["clip_vision_module"] = clip_vision_model.params
...
class FlaxCLIPVisionBertForMaskedLMModule(nn.Module):
    ...
    def setup(self):
        self.model = FlaxCLIPVisionBertModule(config=self.config, add_pooling_layer=False, dtype=self.dtype)
...
# bert_mlm = FlaxCLIPVisionBertForMaskedLM.from_bert_clip_vision_pretrained('bert-base-uncased', 'openai/clip-vit-base-patch32', seed=0, dtype=jnp.float32)
